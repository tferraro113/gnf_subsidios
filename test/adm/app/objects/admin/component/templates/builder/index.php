...
foreach ($this->colsCustom as $k => $v) {
	if ($k == 'action') {
		...
		array_unshift($matches[0], '<a class="btn btn-mini btn-info" href="'.strUrl('admin/scrud/config.php?com_id=').'{crud_components.id}" >Build</a>');
		$clsWidth += 130;
...
?>
    ...
    <?php
     foreach ($this->results as $result) { 
    	$rs1 = null;
    	if (file_exists(__DATABASE_CONFIG_PATH__ . '/' . $config_database['default']["database"]. '/'.sha1('com_'.$result['crud_components']['id']) . '/' . $result['crud_components']['component_table'] . '.php')) {
    		$rs1 = str_replace("<?php exit; ?>\n", "", file_get_contents(__DATABASE_CONFIG_PATH__ . '/' . $config_database['default']["database"]. '/'.sha1('com_'.$result['crud_components']['id']) . '/' . $result['crud_components']['component_table'] . '.php'));
    	}
    	if (!empty($rs1)){
    ?>
