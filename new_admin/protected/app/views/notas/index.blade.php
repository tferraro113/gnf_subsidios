  ...
  <div class="page-content">
    ...
    <div class="breadcrumb-line">
	  ...
	  <ul class="breadcrumb-buttons collapse">
			...
			<li><a href="{{ URL::to('notas/download') }}" class="tips" title="{{ Lang::get('core.btn_download') }}">
			<i class="icon-folder-download2"></i>&nbsp; </a></li>	
			@endif		
		 	@if(Session::get('gid') ==1)
			<li><a href="{{ URL::to('module/config/notas') }}" class="tips"  title="{{ Lang::get('core.btn_config') }}">
			<i class="icon-cog"></i>&nbsp; </a></li>	
			@endif  
			@if($access['is_add'] ==1)
	   		<li>
	   			@if(isset($previous_id))
	   				<a href="{{ URL::to('notas/add?contacto_id='.$previous_id) }}" class="tips"  title="{{ Lang::get('core.btn_create') }}">
	   			@else
	   				<a href="{{ URL::to('notas/add') }}" class="tips"  title="{{ Lang::get('core.btn_create') }}">
	   			@endif	
			<i class="icon-plus-circle2"></i>&nbsp;</a>
		</li>
			@endif  
                ...
                <tr>
				 ...
				 <td>
					...
					{{--*/ $id = SiteHelpers::encryptID($row->id) /*--}}
				 	...
				 	<?php
				 		$url = 'notas/show/'.$id.'?contacto_id='.$row->contacto_id;
				 	?>
					<a href="{{ URL::to($url) }}"  class="tips btn btn-xs btn-default"  title="{{ Lang::get('core.btn_view') }}"><i class="icon-paragraph-justify"></i> </a>
					@endif
					@if($access['is_edit'] ==1)
					<a  href="{{ URL::to('notas/add/'.$id)}}"  class="tips btn btn-xs btn-success"  title="{{ Lang::get('core.btn_edit') }}"> <i class="icon-pencil4"></i></a>
					@endif
...
$(document).ready(function(){
	...
	$('.do-quick-search').click(function(){
		$('#SximoTable').attr('action','{{ URL::to("notas/multisearch")}}');
		$('#SximoTable').submit();
	});
