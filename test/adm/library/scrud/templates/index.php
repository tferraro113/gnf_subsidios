...
<?php 
$buttons = '<div class="btn-group"><a class="btn" onclick="searchModal();"><i class="icon-search"></i> '.$lang->line('search').'</a>';
$q = $this->queryString;
if (isset($q['xtype']))
    unset($q['xtype']);
$buttons .= '<a class="btn" href="?'.http_build_query($q, '', '&').'"><i class="icon-remove-sign"></i></a></div>'." \n";
if (!empty($this->results)) {
    $buttons .= '<a class="btn"  onclick="crudExport();"><i class="icon-download"></i> '.$lang->line('export_csv').'</a>'." \n";
}else{
    $buttons .= '<a class="btn disabled"><i class="icon-download"></i> '.$lang->line('export_csv').'</a>'." \n";
}
$buttons .= '<a class="btn"  onclick="crudExportAll();"><i class="icon-download"></i> '.$lang->line('export_csv_all').'</a>'." \n";
if (in_array(1, $permissions))  {
    $buttons .= '<a type="button" class="btn btn-info" onclick="newRecord();"> <i class="icon-plus icon-white"></i> '.$lang->line('add').'</a>'." \n";
}
        ...
        <form method="post" action="?<?php echo http_build_query($q, '', '&'); ?>" id="table" class="form-horizontal">
            ...
            <?php
                if (is_file($this->conf['theme_path'].'/paginate.php')){
                    require $this->conf['theme_path'].'/paginate.php';
                }else{
                    if (is_file($this->theme_default.'/paginate.php')){
                        require $this->theme_default.'/paginate.php';
                    }
                } 
             ?>
...
<script>
            ...
            function crudExport() {
                $('#crudIframe').attr({src: '<?php echo strUrl('admin/scrud/exportCsv.php?com_id='.$_GET['com_id'].'&xtype=exportcsv'); ?>'});
            }
            function crudExportAll() {
                $('#crudIframe').attr({src: '<?php echo strUrl('admin/scrud/exportCsv.php?com_id='.$_GET['com_id'].'&xtype=exportcsvall'); ?>'});
            }
</script>