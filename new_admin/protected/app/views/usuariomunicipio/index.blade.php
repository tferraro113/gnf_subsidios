  ...
  <div class="page-content">
    ...
    <div class="breadcrumb-line">
	  ...
	  <ul class="breadcrumb-buttons collapse">
			...
			<li><a href="{{ URL::to('usuariomunicipio/download') }}" class="tips" title="{{ Lang::get('core.btn_download') }}">
			<i class="icon-folder-download2"></i>&nbsp; </a></li>	
			@endif		
		 	@if(Session::get('gid') ==1)
			<li><a href="{{ URL::to('module/config/usuariomunicipio') }}" class="tips"  title="{{ Lang::get('core.btn_config') }}">
			<i class="icon-cog"></i>&nbsp; </a></li>	
			@endif  
			@if($access['is_add'] ==1)
	   		<li><a href="{{ URL::to('usuariomunicipio/add') }}" class="tips"  title="{{ Lang::get('core.btn_create') }}">
	...
	<div class="table-responsive">
	 {{ Form::open(array('url'=>'usuariomunicipio/destroy/', 'class'=>'form-horizontal' ,'id' =>'SximoTable' )) }}
            ...
            @foreach ($rowData as $row)
                <tr>
				 ...
				 <td>
					...
					<a href="{{ URL::to('usuariomunicipio/show/'.$id)}}"  class="tips btn btn-xs btn-default"  title="{{ Lang::get('core.btn_view') }}"><i class="icon-paragraph-justify"></i> </a>
					@endif
					@if($access['is_edit'] ==1)
					<a  href="{{ URL::to('usuariomunicipio/add/'.$id)}}"  class="tips btn btn-xs btn-success"  title="{{ Lang::get('core.btn_edit') }}"> <i class="icon-pencil4"></i></a>
					@endif
				...
				@include('usuariomunicipio.inlineview')
            @endforeach
...
$(document).ready(function(){
	...
	$('.do-quick-search').click(function(){
		$('#SximoTable').attr('action','{{ URL::to("usuariomunicipio/multisearch")}}');
		$('#SximoTable').submit();
	});
