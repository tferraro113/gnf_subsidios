...
class Scurd {
    ...
    public function __construct($table = null, $conf = array(), $da = null) {
        $this->theme_default = dirname(__FILE__) . '/templates';
        $hook = Hook::singleton();
        if (empty($da)) {
            die('DataAccess object is not null.');
        }
        
        if (!isset($conf['confirm_page'])){
            $conf['confirm_page'] = 1;
        }
        ...
        if (empty($conf['tool_bar_position'])){
            $conf['tool_bar_position'] = array(50,24);
        }
        
        //if (!function_exists('__toolbar')){
        //    require_once LIBRARY . '/scrud/templates/toolbar.php';
        //}
        if (file_exists($conf['theme_path'].'/toolbar.php')){
            require_once $conf['theme_path'].'/toolbar.php';
        }else{
            require_once $this->theme_default . '/toolbar.php';;
        }
    ...
    private function index() {
        ...
        if ($this->globalAccess == false && 
        	isset($_SESSION['CRUD_AUTH']) && 
        	isset($_SESSION['CRUD_AUTH']['id']) && 
        	in_array($this->conf['table'] . '.created_by', $this->fields) ){
        	$conditions .= ' ' . $strAnd . $this->conf['table'] . '.created_by = '.$_SESSION['CRUD_AUTH']['id'].' ' ;
        	$strAnd = 'AND ';
        }
        
        if (!empty($this->conf['where'])){
            $conditions .= ' ' . $strAnd. $this->conf['where'];
        }
        
        $matches = array();
        preg_match_all("/{([^}]*)}/s", $conditions, $matches);
        if (!empty($matches[0])){
            $user = $_SESSION['CRUD_AUTH'];
            foreach ($matches[1] as $k => $v){
                $v = strtolower($v);
                $ary_v = explode('.', $v);
                $v3 = $user;
                if (count($ary_v) >=2 && $ary_v[0] == 'user'){
                    foreach ($ary_v as  $k1 => $v1){
                        if ($v1 == 'user' ) continue;
                        if (is_array($v3)){
                            $v3 = $v3[$v1];
                        }else{
            ...
            $conditions = str_replace($matches[0], $matches[1], $conditions);
        }
        ...
        if (is_file($this->conf['theme_path'] . '/index.php')) {
            ...
            if (is_file($this->theme_default.'/index.php')){
                require_once $this->theme_default.'/index.php';
            }
        }
    }
    ...
    public function modalform(){
    	...
    	if (is_file($this->conf['theme_path'] . '/search_form.php')) {
    	    ...
    	    if (is_file($this->theme_default.'/search_form.php')){
    	        require_once $this->theme_default.'/search_form.php';
    	        exit;
    	    }
    	}
    }
    ...
    private function exportCsv() {
        ...
        if (is_file($this->conf['theme_path'] . '/csv.php')) {
            ...
            if (is_file($this->theme_default.'/csv.php')){
                require_once $this->theme_default.'/csv.php';
            }
        }
    }

    private function exportcsvall(){
    	$conditions = '';
    	if ($this->globalAccess == false &&
    	isset($_SESSION['CRUD_AUTH']) &&
    	in_array($this->conf['table'] . '.created_by', $this->fields) ){
    		$conditions =  $this->conf['table'] . '.created_by = '.$_SESSION['CRUD_AUTH']['id'].' ' ;
    	}
    ...
    private function form() {
        ...
        if (isset($_GET['key'])) {
            ...
            isset($_SESSION['CRUD_AUTH']) &&
            in_array($this->conf['table'] . '.created_by', $this->fields) ){
            	$strCon .= ' ' . $_tmp . $this->conf['table'] . '.created_by = '.$_SESSION['CRUD_AUTH']['id'].' ' ;
            	$_tmp = 'AND ';
            }
        ...
        if (is_file($this->conf['theme_path'] . '/form.php')) {
            ...
            if (is_file($this->theme_default.'/form.php')){
                require_once $this->theme_default.'/form.php';
            }
        }
    }
    ...
    private function confirm() {
        ...
        if (count($_POST) > 0 && $this->validate()) {
            if ($this->conf['confirm_page'] == 1){
                if (is_file($this->conf['theme_path'] . '/confirm.php')) {
                    require_once $this->conf['theme_path'] . '/confirm.php';
                } else {
                    if (is_file($this->theme_default.'/confirm.php')){
                        require_once $this->theme_default.'/confirm.php';
                    }
                }
            }else{
                $this->update();
    ...
    private function update() {
        ...
        foreach ($this->data[$this->conf['table']] as $k => $v){
        	...
        	if (isset($this->conf['form_elements'][$this->conf['table'].'.'.$k]) &&
        		($this->conf['form_elements'][$this->conf['table'].'.'.$k]['element'][0] == 'image' ||
        				$this->conf['form_elements'][$this->conf['table'].'.'.$k]['element'][0] == 'file') &&
            	$editFlag == false && (empty($v) || trim($v) == '')){
        		$v = $this->conf['form_elements'][$this->conf['table'].'.'.$k]['def_val'];
        	}
            ...
            if ($editFlag) {
                ...
                try {
                    ...
                    if (in_array($this->conf['table'] . '.modified_by', $this->fields)){
                    	$this->data[$this->conf['table']]['modified_by'] = $_SESSION['CRUD_AUTH']['id'];
                    }
                ...
                try {
                    ...
                    if (in_array($this->conf['table'] . '.created_by', $this->fields)){
                    	$this->data[$this->conf['table']]['created_by'] = $_SESSION['CRUD_AUTH']['id'];
                    }
        ...
        if (isset($_GET['key'])) {
            ...
            if (!empty($rs)){
	            ...
	            if (is_file($this->conf['theme_path'] . '/delete_confirm.php')) {
	                ...
	                if (is_file($this->theme_default.'/delete_confirm.php')){
	                    require_once $this->theme_default.'/delete_confirm.php';
	                }
	            }
            }else{
            	$q = $this->queryString;
        ...
        if (isset($_GET['key'])) {
            ...
            if (!empty($rs)){
	            ...
	            if (is_file($this->conf['theme_path'] . '/view.php')) {
	                ...
	                if (is_file($this->theme_default.'/view.php')){
	                    require_once $this->theme_default.'/view.php';
	                }
	            }
            }else{
            	$q = $this->queryString;
