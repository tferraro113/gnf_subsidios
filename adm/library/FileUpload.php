...
class FileUpload {
    ...
    function upload() {
        ...
        if (is_uploaded_file($this->tmpFileName)) {
            if ($this->validateExtension()) {
                ...
                $this->message[] = sprintf($lang->line('only_files_with_extensions_allwed'), implode(" ", $this->extensions));
                return false;
            }
        } else {
        ...
        if ($this->existingFile($new_file)) {
            ...
            if ($this->checkDir($this->uploadDir)) {
                if (@move_uploaded_file($tmp_file, $newfile)) {
                    ...
                    $this->message[] =  sprintf($lang->line('upload_directory_not_allowed_write'), $this->uploadDir);
                    return false;
                }
            } else {
                $this->message[] = sprintf($lang->line('upload_directory_does_not_exist'), $this->uploadDir);
                return false;
            }
        } else {
            $this->message[] = sprintf($lang->line('file_already_exists'), $this->fileName);
            return false;
        }
    }
    ...
    private function errorText($err_num) {
    	$lang = Lang::singleton();
        // start http errors
        $error[1] = $lang->line('upload_file_exceeds_the_max');
        $error[2] = $lang->line('upload_file_exceeds_max_file_size');
        $error[3] = $lang->line('upload_file_was_not_partially_uploaded');
        $error[4] = $lang->line('no_file_was_uploaded');
        // end  http errors
