...
foreach ($this->colsCustom as $k => $v) {
	if ($k == 'action') {
		...
		array_unshift($matches[0], '<a class="btn btn-mini btn-primary" href="'.strUrl('admin/language/translate.php?id=').'{crud_languages.id}">'.$lang->line('translate').'</a>');
		$clsWidth += 60;
    ...
    <script>
                    ...
                    function removeConfig(com_id,com_name){
                    	$.sModal({
                            ...
                            animate: 'fadeDown',
                            content:"Are you sure you want to remove "+com_name+"'s configure?",
                            buttons: [
                                {
